...
export async function GET(req: Request) {
  ...
  if (error || !code || !state) {
    return Response.redirect(`${appUrl}/capabilities?error=github_denied`);
  }
  ...
  try {
    ...
    return Response.redirect(`${appUrl}/capabilities?connected=github`);
  } catch (e) {
    console.error('[github/callback]', e);
    return Response.redirect(`${appUrl}/capabilities?error=github_failed`);
  }
}
